...
import { chat, loadModel, onAgentState } from '@/services/qvac-engine';
...
export default function EdgeAIScreen() {
  ...
  useEffect(() => onAgentState(setAgent), []);
  useEffect(() => {
    loadModel(true);
  }, []);
  ...
  return (
    <SafeAreaView style={s.container}>
      <ChatHeader agent={agent} onBack={() => router.back()} onContacts={openContacts} />
      <KeyboardAvoidingView
        style={{ flex: 1 }}
      ...
      >
        ...
        <Composer value={input} onChange={setInput} onSend={send} disabled={!agent?.loaded} />
      </KeyboardAvoidingView>
...
function ModelChip({ agent }: { agent: AgentState | null }) {
  if (!agent) return null;

  const handlePress = () => {
    if (!agent.loaded && !agent.loading) loadModel();
  };
      ...
      <View style={s.inputRow}>
        <View style={s.inputBox}>
          <TextInput
            ...
            placeholder={disabled ? 'Model loading…' : 'Ask your wallet…'}
